...
class MlpPolicy(object):
    ...
    def _init(self, ob_space, ac_space, hid_size, num_hid_layers, activation_function='tanh', gaussian_fixed_var=True, state_normalization=True, neural_net='normal', nunber_of_obstacles=1):
        ...
        with tf.variable_scope('vf_c0'):
            ...
            self.vpred_c0 = tf.layers.dense(last_out, 1, name='final', kernel_initializer=U.normc_initializer(1.0))[:,0]
        ...
        self._act = U.function([stochastic, ob, portion], [ac, self.vpred, self.vpred_c0, mean_val, std_val, portion_val, logstd_val, pdparam, ob, self.ob_rms.mean, self.ob_rms.std, pol_output])

    def act(self, stochastic, ob, portion=1.0, save_mine=False, nnet_path=''):
        ac1, vpred1, vpred_c0, mean_val, std_val, portion_val, logstd_val, pdparam, ob, ob_rms_mean, ob_rms_std, pol_output =  self._act(stochastic, ob[None], [portion,portion,portion,portion])

        return ac1[0], vpred1[0], vpred_c0[0]
