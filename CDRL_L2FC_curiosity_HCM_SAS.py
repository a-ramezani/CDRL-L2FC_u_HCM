...
class Predictor(object):
    def __init__(self, sess, ob_space, ac_space, reward_space, designHead='universe', parameters='', ac_pdtype=''):
        ...
        if designHead == 'smallNips':
            f_full = inversesmallNipsHead(f, input_shape)
...
class Curiosity_HCM_SAS(object):
    def __init__(self, sess, uid, hidden_layer_size, hidden_layer_number, env_state_space_dim=28, env_action_space='', task=0, visualise=False, unsupType='action', envWrap=False, designHead='universe', noReward=False, parameters='', ac_pdtype=''):
        ...
        self.curiosity_state=self.a_curiosity_observation_space.sample()

        self.curiosity_next_state=self.a_curiosity_observation_space.sample()

        self.ACTION_SIZE=self.a_curiosity_observation_space.sample()

        self.curiosity_action=self.a_action_space_low_level.sample()
        self.curiosity_action2=self.a_action_space_low_level.sample()

        self.curiosity_reward=self.a_reward_space_low_level.sample()
        ...
        policy_number_name='a'
        policy_number='1'

        uid=policy_number_name + policy_number
        ...
        self.predictor = None
        numaction = self.a_action_space_low_level.shape[0] #env.action_space.n
        ...
        for CuH in range(self.parameters.number_of_curiosity_heads):
            with tf.variable_scope("global"):
                ...
                with tf.variable_scope(str(uid) + "predictor" + str(CuH)):
                    self.ap_network.append(Predictor(self.sess, self.a_curiosity_observation_space.shape, self.a_action_space_low_level.shape, self.a_reward_space_low_level.shape, designHead, self.parameters, ac_pdtype))
